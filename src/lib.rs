...
impl ShellIO {
    ...
    pub fn new<I, O>(input: I, output: O) -> ShellIO
        where I: AsyncRead + 'static, O: AsyncWrite + 'static
    {
        return ShellIO {
...
impl AsyncWrite for ShellIO {
    fn shutdown(&mut self) -> Poll<(), std::io::Error> {
        return self.output.lock().expect("Cannot get handle to console output").shutdown();
    }
}
...
impl<T> Shell<T> {
    ...
    pub fn run_loop<R: AsyncRead + 'static, W: AsyncWrite + 'static>(
        ...
        let framed_read = tokio::codec::FramedRead::new(io.clone(), tokio::codec::LinesCodec::new());
            ...
            move |line| -> Box<dyn Future<Item = (), Error = ExecError>> {
                ...
                Box::new(
                    shell
                        .borrow_mut()
                        .eval(&mut io, &line)
                        .then(move |result| -> Box<dyn Future<Item = (), Error = ExecError>> {
                            match result {
                                Ok(_) => {
